		...
		namespace OpTests
		{
			...
			public:
				...
				TEST_METHOD(Test_ld_a_ff_c)
				{
					...
					memory::writeUInt8(address, opcode);
					cpu::registers.c = c;
					memory::writeUInt8(0xff00 + c, expected);
				...
				******************************************************************************************/
				...
				TEST_METHOD(Test_ld_de_a)
				{
					...
					// destination register
					const uint8_t actual = memory::readUInt8(cpu::registers.de);
				...
				TEST_METHOD(Test_ld_nn_a)
				{
					const uint8_t opcode = 0xea;
					const uint8_t expected = rand() % 0xff;
					const uint16_t address = rand() % 0xffff;
					const uint16_t nn_address = rand() % 0xffff;

					using namespace KeemoLib;

					cpu::registers.pc = address;
					memory::writeUInt8(address, opcode);
					memory::writeUInt16(address + 1, nn_address);
					...
					// destination register
					const uint8_t actual = memory::readUInt8(memory::readUInt16(address + 1));
				...
				* LDX r1, r2
				*/
				TEST_METHOD(Test_ldd_a_hl)
				{
					const uint8_t opcode = 0x3a;
					const uint16_t address = rand() % 0xffff;
					const uint8_t expected = rand() % 0xff;
				...
				TEST_METHOD(Test_ldi_hl_a)
				{
					const uint8_t opcode = 0x22;
					const uint16_t address = rand() % 0xffff;
					const uint8_t expected = rand() % 0xff;
					const uint8_t hl = rand() % 0xffff;
				...
				TEST_METHOD(Test_ldh_n_a)
				{
					...
					// we check if for actual
					const uint8_t actual = memory::readUInt8(0xff00 + n);
					Assert::AreEqual(expected, actual);

					// then we check address + 2 (2 bytes op lenght)
					Assert::IsTrue((address+2) == cpu::registers.pc);
				}

				TEST_METHOD(Test_ld_a_ff_n)
				{
					...
					// register preparation
					memory::writeUInt8(0xff00 + n, expected);
